        ...
        <div className="flex items-center gap-2 mb-8">
          {[1, 2, 3].map((s, i) => (
            <div key={s} className="flex items-center">
              <motion.div
                className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold ${step >= s ? 'bg-purple-600' : 'bg-white/20'}`}
              >
          ...
          <AnimatePresence mode="wait">
            {step === 1 && (
              <motion.div key="step1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
                ...
                <p className="text-center text-white/50 text-xs">
                  Have an account? <button onClick={() => navigate('/login')} className="text-purple-400">Login</button>
                </p>
              </motion.div>
            )}
            ...
            {step === 2 && (
              <motion.div key="step2" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
                ...
                <div>
                  ...
                  <Input type="date" value={fatherBirthday} onChange={(e) => setFatherBirthday(e.target.value)} className="bg-white/10 border-white/20 text-white" />
                </div>
                <Button onClick={handleProceedToPayment} disabled={!isStep2Valid} className="w-full bg-cyan-600">
                  Continue <CreditCard className="w-4 h-4 ml-2" />
                </Button>
              </motion.div>
            )}
            ...
            {step === 3 && (
              <motion.div key="step3" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
                ...
                <div className="bg-purple-900/50 rounded-2xl p-6 border border-purple-500/30">
                  <div className="flex justify-between mb-4">
                    <span className="text-white">Baby Prediction</span>
                ...
                <Button onClick={() => navigate('/subscribe')} className="w-full bg-amber-500 text-lg py-6">
                  Pay Now - €10 <ArrowRight className="w-5 h-5 ml-2" />
                </Button>
              </motion.div>
            )}
          </AnimatePresence>
