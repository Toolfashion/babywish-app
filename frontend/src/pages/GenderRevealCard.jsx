...
const GenderRevealCard = ({ prediction, onClose }) => {
  ...
  const shareToSocial = (platform) => {
    ...
    const urls = {
      ...
      instagram: 'https://instagram.com/getbabywish',
      tiktok: 'https://tiktok.com/@getbabywish'
    };
    if (urls[platform]) {
          ...
          <div className="relative z-10 text-center text-white">
            ...
            <div className="mb-6">
              ...
              <div className="flex items-center justify-center gap-2">
                <div className={`text-5xl font-black ${isBoy ? 'text-cyan-300' : 'text-pink-300'}`}>{probability}%</div>
            ...
            <p className="text-white/70 text-sm mb-2">Ανακάλυψε κι εσύ το φύλο του μελλοντικού σου παιδιού!</p>
            <p className={`font-bold ${isBoy ? 'text-cyan-300' : 'text-pink-300'}`}>getbabywish.com</p>

            <div className="mt-6 pt-4 border-t border-white/20">
              <p className="text-xs text-white/50">#BabyWish #GenderReveal #AIprediction</p>
            </div>
          </div>
        </div>

        <div className="mt-6 space-y-4">
          <Button onClick={downloadCard} disabled={downloading} className="w-full bg-gradient-to-r from-cyan-600 to-cyan-700 hover:from-cyan-700 hover:to-cyan-800 py-6 text-lg rounded-xl">
            {downloading ? 'Κατέβασμα...' : <><Download className="mr-2 w-5 h-5" />Κατέβασε για Instagram Story</>}
          </Button>

          <div className="grid grid-cols-5 gap-2">
            <Button onClick={() => shareToSocial('facebook')} className="bg-[#1877F2] hover:bg-[#1864D9] py-4"><Facebook className="w-5 h-5" /></Button>
            <Button onClick={() => shareToSocial('twitter')} className="bg-black hover:bg-gray-800 py-4"><Twitter className="w-5 h-5" /></Button>
            <Button onClick={() => shareToSocial('instagram')} className="bg-gradient-to-br from-purple-600 via-pink-600 to-orange-500 hover:opacity-90 py-4"><Instagram className="w-5 h-5" /></Button>
            <Button onClick={() => shareToSocial('tiktok')} className="bg-black hover:bg-gray-800 py-4">
            ...
            <Button onClick={copyToClipboard} className="bg-gray-700 hover:bg-gray-600 py-4">{copied ? <Check className="w-5 h-5 text-green-400" /> : <Copy className="w-5 h-5" />}</Button>
          </div>

          <Button onClick={onClose} variant="ghost" className="w-full text-white/60 hover:text-white hover:bg-white/10">Κλείσιμο</Button>
        </div>
      </motion.div>
    </motion.div>
  );
