...
import { Star, Moon, Sparkles, LogOut, CreditCard, Clock, Mail } from 'lucide-react';
...
const DashboardPage = () => {
  ...
  useEffect(() => {
    const fetchExistingPrediction = async () => {
      ...
      finally { setCheckingPrediction(false); }
    };
    if (isAuthenticated) fetchExistingPrediction();
  }, [isAuthenticated]);
      ...
      <div className="relative z-20 min-h-screen flex flex-col">
        ...
        <main className="flex-1 flex flex-col items-center justify-center px-4 py-8">
          <AnimatePresence mode="wait">
            {isPendingApproval ? (
              <motion.div key="pending" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full max-w-lg">
                <div className="backdrop-blur-xl bg-white/5 rounded-2xl p-8 text-center border border-white/10">
                  ...
                  <p className="text-white/60 mb-6">Our AI is analyzing your data. You will receive your result via email soon!</p>
              ...
              <motion.div key="result" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full max-w-lg">
                <div className="backdrop-blur-xl bg-white/5 rounded-2xl p-8 text-center border border-white/10">
              ...
              <motion.div key="no-prediction" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full max-w-lg text-center">
                <div className="backdrop-blur-xl bg-white/5 rounded-2xl p-8 border border-white/10">
                  <Sparkles className="w-16 h-16 text-purple-400 mx-auto mb-4" />
                  <h2 className="text-2xl font-bold text-white mb-4">No predicti
